...
def mlp(sizes, activation, output_activation=nn.Identity):
    ...
    for layer in range(len(sizes) - 1):  # note we don't iterate over the output layer
        layerlist.append(nn.Linear(sizes[layer], sizes[layer + 1]))
        if layer < len(sizes) - 2:  # if not the output layer
            layerlist.append(activation())
        else:
            layerlist.append(output_activation())
...
class DiagonalGaussianDistribution:
    ...
    def sample(self):
        ...
        return torch.normal(self.mu, self.log_std.exp())
...
class MLPGaussianActor(nn.Module):
    ...
    def __init__(self, obs_dim, act_dim, hidden_sizes, activation):
        ...
        self.log_std = nn.Parameter(-0.5 * torch.ones(act_dim))  # trainable parameter
